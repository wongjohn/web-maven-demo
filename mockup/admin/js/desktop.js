...
alyDesktop.controller("NewsController", ["$scope", "confirmDialog", "entityEdit", "$http", "Path", "alertMessage", function ($scope, confirmDialog, entityEdit, $http, Path, alertMessage) {
    //公司动态
    var sampleCompanyNew = new CompanyNew({
        newsId: 1,
        shortcut: "ai-le-you-fa-xing-ting-jin-qian-san-jia",
    ...
    $scope.allCompanyNews = [sampleCompanyNew, angular.copy(sampleCompanyNew), angular.copy(sampleCompanyNew), angular.copy(sampleCompanyNew), angular.copy(sampleCompanyNew), angular.copy(sampleCompanyNew)];
    ...
    var _confirmToSaveCompanyNewAdd = function (entity) {//“保存”按钮新增状态的操作
        $scope.allCompanyNews.splice(0, 0, entity);
        $scope.selectedCompanyNew = entity;
    ...
    $scope.selectedCompanyNew = sampleCompanyNew;//选中的记录
    ...
    //媒体动态
    var sampleMediaNew = new MediaNew({
        newsId: 1,
        newsTitle: "《星际猎人》评测：太空与枪战是男人的浪漫",
        referenceLink: "http://www.ptbus.com/xjlr/329794/",
        newsDate: "2015-01-13"
    });
    $scope.allMediaNews = [sampleMediaNew, angular.copy(sampleMediaNew), angular.copy(sampleMediaNew), angular.copy(sampleMediaNew), angular.copy(sampleMediaNew)];
    ...
    var _confirmToSaveMediaNewAdd = function (entity) {//“保存”按钮新增状态的操作
        $scope.allMediaNews.splice(0, 0, entity);
        $scope.selectedMediaNew = entity;
    ...
    $scope.selectedMediaNew = sampleMediaNew;//选中的记录
...
alyDesktop.controller("RecruitController", ["$scope", "confirmDialog", "entityEdit", "$http", "Path", "alertMessage", function ($scope, confirmDialog, entityEdit, $http, Path, alertMessage) {
    ...
    /*查询到的所有招聘信息*/
    $scope.allJobs = [sampleJob, angular.copy(sampleJob), angular.copy(sampleJob), angular.copy(sampleJob), angular.copy(sampleJob), angular.copy(sampleJob), angular.copy(sampleJob)];
    ...
    var _confirmToSaveAdd = function (entity) {//“保存”按钮新增状态的操作
        $scope.allJobs.splice(0, 0, entity);
        $scope.selectedJob = entity;
    ...
    $scope.selectedJob = sampleJob;//选中的记录
...
alyDesktop.controller("GamesController", ["$scope", "confirmDialog", "entityEdit", "$http", "Path", "alertMessage", function ($scope, confirmDialog, entityEdit, $http, Path, alertMessage) {
    /*网络游戏*/
    var sampleOnlineGame = new Games({
        gameId: 1,
        gameType: "网络游戏",
        gameName: "雷霆战机",
    ...
    var defaultOnlineGame = new Games({type: "网络游戏"});
    $scope.allOnlineGames = [sampleOnlineGame, angular.copy(sampleOnlineGame), angular.copy(sampleOnlineGame), angular.copy(sampleOnlineGame), angular.copy(sampleOnlineGame), angular.copy(sampleOnlineGame)];
    ...
    var _confirmToSaveOnlineGameAdd = function (entity) {//“保存”按钮新增状态的操作
        $scope.allOnlineGames.splice(0, 0, entity);
        $scope.selectedOnlineGame = entity;
    ...
    $scope.selectedOnlineGame = sampleOnlineGame;//选中的记录
    ...
    /*休闲游戏*/
    var sampleCasualGame = new Games({
        gameId: 2,
        gameType: "休闲游戏",
        gameName: "大鱼吃小鱼3",
        additionInfo: "休闲游戏",
        description: "看着一道道凶恶的目光，身为小鱼的你要小心啦！看似风平浪静的大海里危机四伏！不但要觅食以填饱肚子，更重要的是学会保护自己！没错，这是一个弱肉强食的世界，拿出你的勇气，将自己进化到食物链的顶点！",
        iconImageUrl: "images/game/dayu.jpg"
    });
    var defaultCasualGame = new Games({type: "休闲游戏"});
    $scope.allCasualGames = [sampleCasualGame, angular.copy(sampleCasualGame), angular.copy(sampleCasualGame), angular.copy(sampleCasualGame), angular.copy(sampleCasualGame), angular.copy(sampleCasualGame)];
    ...
    var _confirmToSaveCasualGameAdd = function (entity) {//“保存”按钮新增状态的操作
        $scope.allCasualGames.splice(0, 0, entity);
        $scope.selectedCasualGame = entity;
    ...
    $scope.selectedCasualGame = sampleCasualGame;//选中的记录
